...
'''
        ...
        for y in self.cratesy:
            ...
            if who=="player":  #remmember we are going 0 (down) to up + and also left 0 to right +
                if self.directions.pl_bullet.y>=int(y) and self.directions.pl_bullet.y<=int(y)+resy/8:
                    ...
                    with self.canvas:
                        #Color(1., 0, 0)
                        Rectangle(source="shadow.png",pos=(self.directions.pl_bullet.x, self.directions.pl_bullet.y), size=(10, 10))
    ...
    def update_player(self,_):
        ...
        if self.player_fire==False:
            ...
            if self.directions.pl_bullet.x<=self.player2.x+70:
                ...
                p_anim_bullet=Animation(x=self.player2.x+80, y=self.directions.pl_bullet.y, angle=0, duration=0.1)
                ...
                if self.directions.pl_bullet.x<=self.player2.x+75:
        ...
        if self.enemy_fire==False:
            self.directions.e_bullet.x=self.player2.x+70
            self.directions.e_bullet.y=self.player2.y+10
            #self.directions.e_bullet.opacity=0
        else:
            if self.directions.e_bullet.x>=self.player.x-30:
                ...
                if self.bullet2_sound==True:
                    if (self.directions.e_bullet.x==self.player2.x+70): 
                        self.bullet2.play()
                        self.bullet2_sound=False
                    else:
